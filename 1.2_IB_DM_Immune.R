...
#IB VS DM ROI:IM area

#G:\IB\231-202306\4T1动物\DSP\测序结果\R\1.DEG_enrichment\1.2_Immune
if(T){
  rm(list = ls())
  setwd("G:\\IB\\231-202306\\4T1动物\\DSP\\测序结果\\R\\1.DEG_enrichment\\1.2_Immune")
  
  #输出文件夹!!!!!!!
  program_name="IB_DM_Immune"
...
load("G:/IB/231-202306/4T1动物/DSP/测序结果/R/1.DEG_enrichment/1.2_Immune/1.2_IB_DM_Immune.RData")
...
gene_volcano <-c(gene_volcano,"Cd274","Pdcd1") #输入兴趣基因
print(gene_volcano)
...
gene_volcano1 =head(DEG[DEG$Group != "Stable",]$Symbol,20) #272
#排序
meta<-arrange(meta,Group)
#添加显著性注释！
row_df<-DEG[rownames(DEG) %in% gene_volcano1,]

hallmark_p <-pheatmap(exprSet[rownames(exprSet) %in% gene_volcano1, rownames(meta)], #gene_volcano
                      ...
                      gaps_col= 11, #table(exp_marker1$response)
                      color= rev(colorRampPalette(brewer.pal(11, "Spectral"))(8)) ,#colorRampPalette( c("navy", "white", "firebrick3"))(10)#,
                      annotation_col=meta[c(32,31,33)],
                      annotation_row = row_df[c(1,4)],annotation_names_row=F
)
hallmark_p
...
save_pheatmap_pdf(hallmark_p, paste0(program_name,"/DEG_genes_pheatmap.pdf"),8,6) 
...
##批量基因boxplot----
gene_more <-unique(c(gene_volcano1,gene_volcano) ) #unique(c(gene_volcano,c("STAT3","STAT4","SLC47A1","SLC7A11","GPX4","CD274"))) #输入基因集
...
if(T){
  ########
  #使用差异基因
  input_gene <-DEG[DEG$Group != "Stable",]$Symbol #283
  gene_type<-"ALL" #上调基因？
  ...
  library(ggplot2)
  ## 请看.csv选择部分通路展示！！！！！！！！！
  #num<-c(49,62,66,81,106,167,181,320,329,418,420,425,447,450,613:616,674,681,697)
  num<-c(66,81,106,167,181,320,329,418,420,425,447,450,613:615,674,681,697)
  
  #dotplot(eego,showCategory = 5)#+facet_grid(ONTOLOGY~., scale="free")#点状图
  dotplot(eego,color = "pvalue", #p.adjust title = 'GO',
          showCategory = c(df_go$Description[num] )
  )+
    scale_color_continuous(low='pink', high='#57C3F3')
  ggsave(paste0(program_name,"/DEG_",gene_type,"_GO_my-1.pdf"),width = 6,height = 8,family="serif")
  ...
  for (i in c("RdPu","YlOrRd","PiYG") ){
    ...
    ggsave(paste0(program_name,"/DEG_",gene_type,"_GO_ggplot2_",i,".pdf"),family="serif",width = 6,height = 4)
  ...
  for (i in c("RdPu","YlOrRd","PiYG") ){
    ...
    ggplot(data = ego1,
      ...
      geom_text(aes(x=0.03, # 增大与条形图的间距
                ...
                size = 3.5) +
      theme_classic() +theme(axis.text.y = element_blank(),axis.ticks.y = element_blank() )
    ggsave(paste0(program_name,"/DEG_",gene_type,"_KEGG_bar_",i,".pdf"),family="serif",width = 5,height = length(ego1$Description)/2 )
    ...
    p
    ggsave(paste0(program_name,"/DEG_",gene_type,"_KEGG_dotplot_",i,".pdf"),family="serif",width = 5.5,height = length(ego1$Description)/2 ) #
  ...
  p
  ggsave(paste0(program_name,"/DEG_",gene_type,"_KEGG_bar+genes.pdf"),width = 6,height = length(ego1$Description)/2,family="serif")#7.5
  ...
  gseaplot2(
    ...
    geneSetID = c(1:length(kkgsea_res$ID)),#"hsa00310",#1,#"hsa05168",#富集的ID编号
    title = "", #标题
    color = "red",#GSEA线条颜色
    base_size = 11,#基础字体大小
  ...
  library(pheatmap);library(RColorBrewer)
  pheat <-pheatmap(as.data.frame(t(pathways[rownames(meta),1:14])),show_colnames = F, #col sample; row genes
                   cluster_rows = T,cluster_cols = F,border_color = NA,gaps_col = as.numeric(table(pathways$Group)[1]) ,
                   annotation_col=pathways[c(45,47)],#x, 
  ...
  save_pheatmap_pdf(pheat, paste0(program_name,"/progeny_pheatmap_1.pdf"),4.5,2.5) 
...
exp<-exp[,colnames(exp) %in% meta$id]#30
##all sample
if(T){
  #exp meta #95
  ...
  ##single boxplot
  library(reshape2)
  bar_df <-melt(meta1[c(37:38,34)],id.vars=c("Group") ) #38:39"Infiltration_score" 74
  for (i in as.character(unique(bar_df$variable)) ){
    for (j in p_method ){ #wilcox.test
      library(ggplot2);library(ggpubr)
