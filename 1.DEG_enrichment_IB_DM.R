...
###202503 Powered by M.D. Jianbo Zhou, West China Hospital, Sichuan University

if(T){
  rm(list = ls())
  setwd("G:\\IB\\231-202306\\4T1动物\\DSP\\测序结果\\R\\1.DEG_enrichment\\1.0_IB_DM_all")
  
  #输出文件夹!!!!!!!
  program_name="IB_DM_all"
...
load("G:/IB/231-202306/4T1动物/DSP/测序结果/R/1.DEG_enrichment/1.0_IB_DM_all/.RData")
...
#分组信息
# group <-data.frame(name=c("NC1","NC2","NC3","OE1","OE2","OE3"),#样品名称
...
fviz_pca_ind(data.pca, 
             ...
             title="",legend.title="Group", 
             palette = c("#00AFBB", "#E7B800", "#FC4E07"))+  #"skyblue", "pink","#7EC7A7"
  theme(panel.grid = element_blank(),
        panel.border = element_rect(fill=NA,color="black", size=0.1, linetype="solid"))#加个边框

#ggsave(paste0(program_name,"/PCA_IB_DM.pdf"),width = 3.5,height = 3,family="serif")
#效果差，删除样本？或者取差异基因？http://www.bio-info-trainee.com/9562.html
#ggsave(paste0(program_name,"/PCA_IB_DM_TC_IM.pdf"),width = 4,height = 3,family="serif")
#较开
...
##volcano----
gene_volcano <-  DEG[abs(DEG$logFC) >1.5,]$Symbol #DEG[DEG$Group != "Stable",]$Symbol#DEG[DEG$Group != "Stable",]$Symbol#rownames(DEG[abs(DEG$logFC) >1.5,])#火山图展示基因 
...
#devtools::install_local("ggVolcano.zip")
...
#colnames(deg_data);colnames(data)
#colnames(data)[7]<-"row"
...
gradual_volcano(data, 
                ...
                label = "row", label_number = 5, output = FALSE)+
...
gene_more <-c("Prf1", "Ifng", "Gzmb", "Tnf",#effector function
              "Pdcd1", "Lgals1", "Tox" #T cell exhaustion 
              )
for (i in gene_more ){
  print(i)
  ...
  ggplot(df_gene,#
   ...
   ggsave(paste0(program_name,"/boxplot_",i,"_wilcox_All.pdf"),width = 2,height = 3,family="serif")
...
dotplot(eego,color = "pvalue", #p.adjust title = 'GO',
        showCategory = c(df_go$Description[c(1:4,15,37,65,92,93,122,155,229,237,239,267,297,357,650,652,659)] )
)+
...
for (i in c("RdPu","YlOrRd","PiYG") ){
  print(i)
  ggplot(data = df_go[c(1:4,15,37,65,92,93,122,155,239,267,650,652,659),], 
...
pdf(paste0(program_name,"/DEG_",gene_type,"_KEGG_cnetplot.pdf"),family="serif",width = 6,height = 4)
...
#选择通路
ego1<-ego[c(6,27,29,31,32,65,73,79,87),]
...
# p<- ggplot(data = ego[c(6,27,29,31,32,65,73,79,87),],
...
ggplot(data = ego1,
       aes(x = Count, y = Description, fill =-log10(pvalue) )) +  #Description, fill = Cluster
  scale_fill_distiller(palette = "RdPu",direction = -1) + #RdPu YlOrRd PiYG#scale_fill_distiller(palette = i,direction = 1) + #RdPu YlOrRd PiYG
  geom_bar(stat = "identity", width = 0.7, alpha = 0.5) +
  labs(x = "Count", y = "", title = "KEGG") +
  geom_text(aes(x=0.03, # 增大与条形图的间距
                label = Description,
            ...
            size = 3.5) +
  theme_classic() +theme(axis.text.y = element_blank(),axis.ticks.y = element_blank() )
ggsave(paste0(program_name,"/DEG_",gene_type,"_KEGG_bar.pdf"),family="serif",width = 5,height = 4)

#2.2 dotplot
p<- ggplot(data = ego1,
           aes(x = Count, y = reorder(Description,-Count) ) )+ #, fill = -log10(pvalue)
  scale_color_distiller(palette = "YlOrRd",direction = -1) + #RdPu YlOrRd PiYG
  ##PRGn紫色绿色 #PiYG 紫色绿色  RdYlGn橙绿 
  #geom_bar(stat = "identity", width = 0.8) +
...
p
ggsave(paste0(program_name,"/DEG_",gene_type,"_KEGG_dotplot.pdf"),family="serif",width = 5.5,height = 3.5)
...
library(RColorBrewer)
p <- ggplot(data = ego1, aes(x = -log10(pvalue), y = Description, fill =Description )) +  #, fill = Cluster
...
# ## 最后一步 开始绘图
# plotGseaTable(sig$pathway,genelist_1, fgseaRes,gseaParam = 0.5)
# # 
# # plotGseaTable(fgseaRes[fgseaRes$pval < 0.05,]$pathway[1:10], #显示通路
# #               genelist_1, fgseaRes,gseaParam = 1)
# # plotGseaTable(fgseaRes[fgseaRes$pval < 0.05,]$pathway[1:10],genelist_1,fgseaRes,gseaParam = 1)
# 
# library(export) #保存为PPT后调整
...
#2.5 HALLMARK-ssGSEA-group-pheatmap----
#仅适用于人的数据！
...
library(GSVA)
hallmark50_score <- gsva(as.matrix(exprSet), #[,-1]
                         geneset, #基因集
...
# design <- model.matrix(~0+factor(group$group))
# colnames(design) <- levels(factor(group$group))
# rownames(design) <- colnames(hallmark50_score)
# contrast.matrix <- makeContrasts(contra,levels = design)
fit <- lmFit(hallmark50_score,design) #非线性最小二乘法
fit2 <- contrasts.fit(fit, contrast.matrix)
fit2 <- eBayes(fit2)#用经验贝叶斯调整t-test中方差的部分
DEG <- topTable(fit2, coef = 1,n = Inf,sort.by="logFC")
DEG <- na.omit(DEG)
...
##plot-VlnPlot----
# 看看结果
# VlnPlot(sce,group.by = "TMA.position", #样本
#         features = colnames(meta[13:16]), #纵坐标
#         ncol = 4,log = T,pt.size =0,cols = colors
# )&labs(x="")
# ggsave(paste0(program_name,"/VlnPlot_.pdf"),width = 6,height = 3,family="serif")
# VlnPlot(sce,group.by = "Group", #样本
#         features = colnames(meta[13:16]), #纵坐标
#         ncol = 4,log = T,pt.size =0,cols = colors
# )&labs(x="")
# 
# VlnPlot(sce,group.by = "Type", #样本
#         features = colnames(meta[13:16]), #纵坐标
#         ncol = 4,log = T,pt.size =0,cols = colors
# )&labs(x="")
...
geneset_mouse<-data.frame(
  genes=c("Prf1", "Ifng", "Gzmb", "Tnf"),
  Pathway=c(rep("T_cell_cytotoxicity",4))
  #T_cell_cytotoxicity=c("Pdcd1", "Lgals1", "Tox")
)
geneset_mouse<-rbind(
  geneset_mouse,
  data.frame(
    genes=c("Pdcd1", "Lgals1", "Tox"),
    Pathway=c(rep("T_cell_exhaustion",3))
  )
)
geneset_mouse<-split(geneset_mouse$genes,geneset_mouse$Pathway)
...
if(T){
  ...
  ##single boxplot
  library(reshape2)
  bar_df <-melt(meta1[c(38:39,34)],id.vars=c("Group") ) #38:51 "Infiltration_score" 74
