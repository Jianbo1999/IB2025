...
#G:\IB\231-202306\4T1动物\DSP\测序结果\R\1.DEG_enrichment\1.3_Tumor_Immune
#IB VS DM ROI:tumor-IM 

if(T){
  rm(list = ls())
  setwd("G:\\IB\\231-202306\\4T1动物\\DSP\\测序结果\\R\\1.DEG_enrichment\\1.3_Tumor_Immune")
  
  #输出文件夹!!!!!!!
  program_name="IB_DM_Tumor_Immune"
...
load("G:/IB/231-202306/4T1动物/DSP/测序结果/R/1.DEG_enrichment/1.3_Tumor_Immune/1.3_Tumor_Immune.RData")
...
##批量基因boxplot----
gene_more <-unique(c("Stat3",gene_volcano1,gene_volcano) ) #unique(c(gene_volcano,c("STAT3","STAT4","SLC47A1","SLC7A11","GPX4","CD274"))) #输入基因集
gene_more
for (i in gene_more ){
  print(i)
...
if(T){
  ...
  num<-c(171,173,233,242,261,333,337,454,592,599,695)
  ...
  ggsave(paste0(program_name,"/DEG_",gene_type,"_GO_my-1.pdf"),width = 6,height = length(num)*0.5,family="serif")
  ...
  for (i in c("RdPu","YlOrRd","PiYG") ){
    ...
    ggsave(paste0(program_name,"/DEG_",gene_type,"_GO_ggplot2_",i,".pdf"),family="serif",width = 6,height = length(num)*0.4 )
  ...
  pdf(paste0(program_name,"/DEG_",gene_type,"_KEGG_cnetplot.pdf"),family="serif",width = 12,height = 6)
